        ...
        if(for.paper==T){
            if(show.calibration.data==T){
                plot = ggplot() + 
                    ...
                    geom_line(data = df.sim, aes(x = year, y = value, color = sim.id, group = group.id), lwd=0.35,show.legend = F) + # changed lwd from 1.25 here
                    geom_point(data = df.truth, aes(x = year, y = value, color = sim.id, group = group.id, shape = split), 
...
simplot.basic = function(..., 
    ...
    for(d in data.types){
        ...
        # Extract the data from simulation
        value = as.numeric(get.surveillance.data(data.manager = data.manager, years = years, data.type=d,locations = location))
