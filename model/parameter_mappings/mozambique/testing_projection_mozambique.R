...
get.testing.model.mozambique = function(){
    ...
    testing.data.years =c(2003,2011,2022)-testing.anchor.year
    testing.data.ages = c(17,22,27,35,45) # midpoint of age groups:  15-19, 20-24, 25-29, 30-39, 40-49

    prop.2003.female = c(.035,.031,.021,.023,.008) 
    prop.2003.male = c(.018,.048,.028,.027,.020) 
    prop.2011.female = c(.178,.357,.341,.285,.139) 
    prop.2011.male = c(.077,.164,.194,.163,.134) 
    prop.2022.female = c(.213,.323,.316,.312,.222) 
    prop.2022.male = c(.115,.296,.328,.340,.272) 
    ...
    master.df = data.frame(year=rep(testing.data.years,each=2*length(testing.data.ages)),
                           prop = c(prop.2003.female,prop.2003.male,
                                    prop.2011.female,prop.2011.male,
