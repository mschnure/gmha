...
do.extract.4D <- function(sim,
    ...
    dim.names.with.location = c(dimnames(arr),
                                "location"=location)
    
    arr = array(arr,
                dim = sapply(dim.names.with.location,length),
                dimnames = dim.names.with.location)
    ...
    if(length(ages)==length(unlist(age.brackets)) && all(ages==unlist(age.brackets))){
        ...
        x=arr[as.character(years),ages,sexes,subgroups, hiv.status,location]
        ...
        y=arr[as.character(years),ages.to.pull,sexes,subgroups, hiv.status,location]
        ...
        if(any(keep.dimensions=="age")){
            ...
            for (i in 1:length(ages)){
                ...
                x[,i,,,,] = apply(sub.y, c("year","sex","subgroup","hiv.status","location"), sum)
            }
        }
        else
            x=y
...
do.extract.3D <- function(sim,
    ...
    if(length(ages)==length(unlist(age.brackets)) && all(ages==unlist(age.brackets))){
        #filter the input array for selected years and attributes
        x=arr[as.character(years),ages,sexes,subgroups,location]
        ...
        y=arr[as.character(years),ages.to.pull,sexes,subgroups,location]
        y.dim.names = full.dim.names
        y.dim.names$age  = ages.to.pull
        dim(y) = sapply(y.dim.names, length)
        dimnames(y) = y.dim.names
        ...
        if(any(keep.dimensions=="age")){
            ...
            for (i in 1:length(ages)){
                ...
                x[,i,,,] = apply(sub.y, c("year","sex","subgroup","location"), sum)
            }
        }
        else
            x=y
