...
scale.calibration.data = function(data.type,
                                  location){
    dim.names.1 = dimnames(DATA.MANAGER[[data.type]]$year.age.location)
    
    if(!grepl("hiv.mortality",data.type)) # no age/sex for hiv.mortality
        dim.names.2 = dimnames(DATA.MANAGER[[data.type]]$year.age.sex.location)
    ...
    if(!grepl("hiv.mortality",data.type)) {
        scaling.factor.2 = c(NEW.15.AND.OVER[dim.names.2$year]/apply(DATA.MANAGER[[data.type]]$year.age.sex.location[,,,location],c(1:2),sum))
        
        NEW.15.AND.OVER.BY.SEX = DATA.MANAGER[[data.type]]$year.age.sex.location*scaling.factor.2
    }
    ...
    rv$year.age.location[,,location] = year.age
    dimnames(rv$year.age.location) = dimnames(DATA.MANAGER[[data.type]]$year.age.location)
    
    if(!grepl("hiv.mortality",data.type)) {
        rv$year.age.sex.location = NEW.15.AND.OVER.BY.SEX    
    }
