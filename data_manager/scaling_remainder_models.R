...
# locations = c("unaids.remainder","non.unaids.remainder","r1.low","r1.lower.middle","r1.upper.middle","r1.high")
# location = locations[5]

scale.calibration.data = function(data.type,
                                  location){
    
    if(!data.type %in% c("incidence","prevalence",
                         "incidence.lowers","incidence.uppers",
                         "prevalence.lowers","prevalence.uppers"))
        stop("only scaling incidence and prevalence (and lowers/uppers)")
    if(!location %in% c("unaids.remainder","non.unaids.remainder","r1.low","r1.lower.middle","r1.upper.middle","r1.high"))
        stop("only scaling remainder models")
    ...
    NEW.10.TO.19 = DATA.MANAGER[[data.type]]$year.age.location[,"10-19",location]*mean.scaling.factor
    NEW.15.TO.24 = DATA.MANAGER[[data.type]]$year.age.location[,"15-24",location]*mean.scaling.factor
    
    rv = DATA.MANAGER[[data.type]]$year.age.location[,,location]
    
    rv = cbind(NEW.0.TO.14,
               NEW.10.TO.19,
               NEW.15.TO.24,
               NEW.15.TO.49,
               NEW.15.AND.OVER,
               NEW.50.AND.OVER,
               DATA.MANAGER[[data.type]]$year.age.location[,"All ages",location])
    dimnames(rv) = dimnames(DATA.MANAGER[[data.type]]$year.age)
